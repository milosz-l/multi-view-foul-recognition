...
class WeightedAggregate(nn.Module):
    ...
    def forward(self, mvimages):
        ...
        aux = self.lifting_net(unbatch_tensor(self.model(batch_tensor(
        ...
        aux_sum = torch.sum(torch.reshape(
            relu_res, (B, V*V)).T, dim=0).unsqueeze(0)
        ...
        final_attention_weights = torch.reshape(
            final_attention_weights, (B, V, V))

        final_attention_weights = torch.sum(final_attention_weights, 1)

        output = torch.mul(
            aux.squeeze(), final_attention_weights.unsqueeze(-1))
